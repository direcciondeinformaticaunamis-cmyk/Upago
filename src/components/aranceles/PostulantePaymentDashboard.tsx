...
const PostulantePaymentDashboard: React.FC<Props> = ({
    postulanteName = 'Postulante',
    postulanteCedula,
    ...
    const loadPayments = async () => {
        if (!postulanteCedula) return;
        setIsLoading(true);
        try {
            const data = await FinanceService.getPagos();
            // Filtrar por la cédula del postulante logueado
            const postulantePayments = data.filter(p => p.postulante_cedula === postulanteCedula);
            setPayments(postulantePayments);
        } catch (error) {
            console.error('Error loading postulante payments:', error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="w-full space-y-8">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pl-3">
                        <span className="flex items-center gap-2 text-slate-500 text-sm font-medium">
                            <User size={14} className="text-[var(--primary)]" /> {postulanteName}
                        </span>
                        <span className="w-1 h-1 rounded-full bg-slate-300" />
                        <span className="flex items-center gap-2 text-slate-500 text-sm">
                            <School size={14} className="text-[var(--primary)]" /> {carrera}
                        </span>
                        <span className="w-1 h-1 rounded-full bg-slate-300" />
                        <span className="font-mono text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md">CI: {postulanteCedula}</span>
                    </div>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={() => setShowUploadModal(true)}
                        className="flex items-center gap-2 px-5 py-3 bg-[var(--primary)] text-white rounded-2xl font-bold text-sm shadow-lg shadow-[var(--primary)]/25 hover:bg-[var(--primary-dark)] active:scale-95 transition-all"
                    >
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm group hover:bg-[var(--primary)] transition-all duration-300 cursor-default">
                    <div className="flex justify-between items-start mb-4">
                        <div className="p-2 bg-red-50 rounded-xl group-hover:bg-white/15 transition-colors">
                            <Wallet size={20} className="text-[var(--primary)] group-hover:text-white transition-colors" />
                        </div>
                                ...
                                {payments.map((p) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-5">
                                            <p className="text-sm font-black text-[var(--primary)]">{p.concepto}</p>
                                        ...
                                        <td className="pl-4 pr-8 py-5 text-right">
                                            <button className="text-[var(--primary)] p-2 rounded-xl hover:bg-red-50 transition-colors">
                                                <Eye size={18} />
                                            </button>
                                        </td>
                                    </motion.tr>
                            ...
                            <div className="p-6 space-y-5">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <p className="text-3xl font-black text-[var(--primary)] tracking-tighter">{formatGs(selected.monto)}</p>
                                    </div>
                                </div>
                    ...
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        >
                            <div className="p-8 text-center">
                                <div className="w-16 h-16 bg-red-50 rounded-3xl flex items-center justify-center mx-auto mb-6 text-[var(--primary)]">
                                    <Upload size={32} />
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full p-4 bg-[var(--primary)] text-white rounded-2xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-[var(--primary-dark)] transition-colors"
                                >
...
export default PostulantePaymentDashboard;
